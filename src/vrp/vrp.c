...
int ypos(int i, int j, instance *inst)
{
	if (i == j)
		return -1;
	if (i > j)
		return ypos(j, i, inst);
	return inst->ystart + i * (inst->nnodes + 1) + j - (i + 1) * (i + 2) / 2;
}
...
/**************************************************************************************************************************/
{
	...
	{
		inst->load_min[h] = inst->demand[h];
		inst->load_max[h] = inst->capacity;
	}
	...
	{
		if (VERBOSE >= 2)
			printf(" ... Cplex in opportunistic mode with %d thread(s)\n", inst->num_threads);
	...
	if (inst->randomseed != 0)
	{
		srand(7645321 + abs(inst->randomseed));
	...
	if (CPXsetstrparam(env, CPX_PARAM_WORKDIR, inst->node_file))
		print_error("wrong node file");			   // node file name ("." or "/ext" etc.)
	CPXsetintparam(env, CPX_PARAM_NODEFILEIND, 3); // node file on disk and compressed
	if (strcmp(inst->node_file, "NULL") == 0)
	{
		if (VERBOSE >= 20)
			printf(" ... using cplex without node file: will stop if more than %lf MB\n", inst->available_memory + 0.0);
	...
	if (VERBOSE >= 20)
		printf(" ... using cplex node file %s after %lf MB\n", inst->node_file, inst->available_memory + 0.0);
	...
	// cutoff
	if (inst->cutoff < CPX_INFBOUND / 2.0)
		CPXsetdblparam(env, CPX_PARAM_CUTUP, inst->cutoff);
	...
	else
	{
		...
		if (time_limit_expired(inst))
			goto EXIT;
		if (VERBOSE >= 100)
			CPXwriteprob(env, lp, "final.lp", NULL);
		...
		if (inst->max_nodes >= 0)
			CPXsetintparam(env, CPX_PARAM_NODELIM, inst->max_nodes);
		CPXmipopt(env, lp);
	}
...
/**************************************************************************************************************************/
{
	...
	for (int j = 0; j < ncols; j++)
		CPXchgobj(env, lp, 1, &j, &zero);

	if (VERBOSE >= 100)
		CPXwriteprob(env, lp, "pre.lp", NULL);
	...
	while (inst->bigqstart >= 0)
	{
		if (++h >= inst->nnodes)
			h = 0;
		counter++;
		if (inst->load_min[h] >= inst->load_max[h] - EPSILON)
			continue;
		if (h == last_changed || counter > inst->nnodes)
			break;
		...
		if (VERBOSE >= 20)
			printf("preprocessing: minimum load for node %4d from %10.1lf to %10.1lf after %7.1lf sec.s ####", h + 1, inst->load_min[h], lb, second() - t1);
...
/**************************************************************************************************************************/
{
	...
	if (inst->load_max[inst->depot] > 1e-20)
		print_error(" wrong inst->load_max[inst->depot]");

	// add continuous var.s bigq(i) = load when leaving node i
	if (inst->bigqstart != -1)
		print_error(" ... error in build_model(): var. bigq cannot be redefined!");
	...
	for (int i = 0; i < inst->nnodes; i++)
	{
		...
		if (CPXnewcols(env, lp, 1, &obj, &lb, &ub, &continuous, cname))
			print_error(" wrong CPXnewcols on bigq var.s");
		// if ( CPXnewcols(env, lp, 1, &obj, &lb, &ub, &integer, cname) ) print_error(" wrong CPXnewcols on bigq var.s");
		if (CPXgetnumcols(env, lp) - 1 != bigqpos(i, inst))
			print_error(" wrong position for bigq var.s");
	}

	// add continuous var.s q(i,j) = bigq(i) * x(i,j)
	if (inst->qstart != -1)
		print_error(" ... error in build_model(): var. q cannot be redefined!");
	...
	for (int i = 0; i < inst->nnodes; i++)
	{
		for (int j = 0; j < inst->nnodes; j++)
		{
			...
			if (CPXnewcols(env, lp, 1, &obj, &zero, &ub, &continuous, cname))
				print_error(" wrong CPXnewcols on q var.s");
			// if ( CPXnewcols(env, lp, 1, &obj, &zero, &ub, &integer, cname) ) print_error(" wrong CPXnewcols on q var.s");
			if (CPXgetnumcols(env, lp) - 1 != qpos(i, j, inst))
				print_error(" wrong position for q var.s");
		}
	}
	...
	for (int h = 0; h < inst->nnodes; h++) // out-degree
	{
		...
		if (rhs < 0.1)
			continue; // means that the n. of vehicles is not fixed
		char sense = 'E';
	...
	for (int h = 0; h < inst->nnodes; h++) // in-degree
	{
		...
		if (CPXnewrows(env, lp, 1, &rhs, &sense, NULL, cname))
			print_error(" wrong CPXnewrows [x2]");
		for (int i = 0; i < inst->nnodes; i++)
		{
			if (CPXchgcoef(env, lp, lastrow, xpos(h, i, inst), 1.0))
				print_error(" wrong CPXchgcoef [x2]");
		}
	}
	...
	for (int h = 0; h < inst->nnodes; h++) // define bigq(h) = \sum_j q(h,j)
	{
		...
		if (CPXnewrows(env, lp, 1, &rhs, &sense, NULL, cname))
			print_error(" wrong CPXnewrows [q1]");
		if (CPXchgcoef(env, lp, lastrow, bigqpos(h, inst), -1.0))
			print_error(" wrong CPXchgcoef [q1]");
		for (int j = 0; j < inst->nnodes; j++)
			if (CPXchgcoef(env, lp, lastrow, qpos(h, j, inst), 1.0))
				print_error(" wrong CPXchgcoef [q2]");
	}
	for (int h = 0; h < inst->nnodes; h++) // flow constraint bigq(h) = demand(h) + \sum_i q(i,h)
	{
		...
		if (CPXnewrows(env, lp, 1, &rhs, &sense, NULL, cname))
			print_error(" wrong CPXnewrows [bq1]");
		if (CPXchgcoef(env, lp, lastrow, bigqpos(h, inst), 1.0))
			print_error(" wrong CPXchgcoef [bq1]");
		for (int i = 0; i < inst->nnodes; i++)
			if (CPXchgcoef(env, lp, lastrow, qpos(i, h, inst), -1.0))
				print_error(" wrong CPXchgcoef [bq2]");
	}
	...
	for (int i = 0; i < inst->nnodes; i++)
	{
		...
		for (int j = 0; j < inst->nnodes; j++) // q(i,j) [= bigq(i) * x(i,j)] <= [capacity-demand(j)] x(i,j)
		{
			...
			if (j != inst->depot)
				coef = dmin(coef, inst->capacity - inst->demand[j]);
			if (CPXnewrows(env, lp, 1, &rhs, &sense, NULL, cname))
				print_error(" wrong CPXnewrows [l1]");
			if (CPXchgcoef(env, lp, lastrow, qpos(i, j, inst), 1.0))
				print_error(" wrong CPXchgcoef [l2]");
			if (CPXchgcoef(env, lp, lastrow, xpos(i, j, inst), -coef))
				print_error(" wrong CPXchgcoef [l3]");
		}
	}
	for (int i = 0; i < inst->nnodes; i++)
	{
		...
		for (int j = 0; j < inst->nnodes; j++) // q(i,j) [= bigq(i) * x(i,j)] >= load_min(i) x(i,j)
		{
			...
			if (CPXchgcoef(env, lp, lastrow, qpos(i, j, inst), 1.0))
				print_error(" wrong CPXchgcoef [l2]");
			if (CPXchgcoef(env, lp, lastrow, xpos(i, j, inst), -inst->load_min[i]))
				print_error(" wrong CPXchgcoef [l3]");
		}
	}
...
/**************************************************************************************************************************/
{
	...
	if (inst->sstart != -1)
		print_error(" ... error in build_model(): var. s cannot be redefined!");
	...
	for (int i = 0; i < inst->nnodes; i++)
	{
		for (int j = 0; j < inst->nnodes; j++)
		{
			...
			if (CPXnewcols(env, lp, 1, &sobj, &zero, &ub, &continuous, cname))
				print_error(" wrong CPXnewcols on s var.s");
			if (CPXgetnumcols(env, lp) - 1 != spos(i, j, inst))
				print_error(" wrong position for s var.s");
		}
	}
	...
	if (inst->bigsstart != -1)
		print_error(" ... error in build_model(): var. bigs cannot be redefined!");
	...
	for (int i = 0; i < inst->nnodes; i++)
	{
		...
		double ub = (i == inst->depot) ? 0.0 : inst->load_max[i] * inst->load_max[i];
		if (CPXnewcols(env, lp, 1, &obj, &lb, &ub, &continuous, cname))
			print_error(" wrong CPXnewcols on bigs var.s");
		if (CPXgetnumcols(env, lp) - 1 != bigspos(i, inst))
			print_error(" wrong position for bigs var.s");
	}
	...
	for (int h = 0; h < inst->nnodes; h++) // bigs(h) = \sum_j s(h,j)
	{
		...
		if (CPXnewrows(env, lp, 1, &rhs, &sense, NULL, cname))
			print_error(" wrong CPXnewrows [s1]");
		if (CPXchgcoef(env, lp, lastrow, bigspos(h, inst), -1.0))
			print_error(" wrong CPXchgcoef [s1]");
		for (int j = 0; j < inst->nnodes; j++)
			if (CPXchgcoef(env, lp, lastrow, spos(h, j, inst), 1.0))
				print_error(" wrong CPXchgcoef [s2]");
	}
	for (int h = 0; h < inst->nnodes; h++) //  bigs(h) = (demand[h] + \sum_i q(i,h))^2 = demand[h]^2 + sum_i s(i,h) + 2 * demand[h] * sum_i q(i,h))
	{
		...
		for (int i = 0; i < inst->nnodes; i++)
		{
			if (CPXchgcoef(env, lp, lastrow, spos(i, h, inst), -1.0))
				print_error(" wrong CPXchgcoef [bq2]");
			if (CPXchgcoef(env, lp, lastrow, qpos(i, h, inst), -2.0 * inst->demand[h]))
				print_error(" wrong CPXchgcoef [bq2]");
		}
	}
#endif
	...
	for (int i = 0; i < inst->nnodes; i++)
	{
		...
		for (int j = 0; j < inst->nnodes; j++)
		{
			...
			quadrow[0] = quadcol[0] = qpos(i, j, inst);
			quadrow[1] = spos(i, j, inst);
			quadcol[1] = xpos(i, j, inst);
			if (CPXaddqconstr(env, lp, linnzcnt, quadnzcnt, rhsval, sense, linind, linval, quadrow, quadcol, quadval, cname[0]))
				print_error(" wrong CPXaddqconstr [1]");
		}
	}
...
/**************************************************************************************************************************/
{
	...
	if (CPXnewcols(env, lp, 1, &zero, &llb, &uub, &integer, cname))
		print_error(" wrong CPXnewcols on nveh var");
	if (CPXgetnumcols(env, lp) - 1 != nvehpos)
		print_error(" wrong position for nveh var.s");
	...
	for (int i = 0; i < numext; i++)
	{
		...
		for (int j = i + 1; j < numext; j++)
		{
			...
			if (CPXnewcols(env, lp, 1, &obj, &zero, &ub, &type, cname))
	...
	for (int i = 0; i < numext; i++)
	{
		for (int j = 0; j < numext; j++)
		{
			...
			if (i == inst->depot && j == numext - 1)
				ub = 0;
			if (j == inst->depot && i == numext - 1)
				ub = 0;
			if (CPXnewcols(env, lp, 1, &obj, &zero, &ub, &continuous, cname))
	...
	for (int i = 0; i < inst->nnodes; i++)
	{
		...
		if (CPXnewrows(env, lp, 1, &rhs, &sense, NULL, cname))
			print_error(" wrong CPXnewrows [f1]");
		for (int j = 0; j < numext; j++)
		{
			if (CPXchgcoef(env, lp, lastrow, fpos(j, i, inst), +1.0))
				print_error(" wrong CPXchgcoef [f3]");
			if (CPXchgcoef(env, lp, lastrow, fpos(i, j, inst), -1.0))
				print_error(" wrong CPXchgcoef [f2]");
		}
	}
	...
	// eq. (29)
	{
		...
		if (CPXnewrows(env, lp, 1, &rhs, &sense, NULL, cname))
			print_error(" wrong CPXnewrows [f1a]");
		for (int j = 0; j < inst->nnodes; j++)
			if (j != inst->depot && CPXchgcoef(env, lp, lastrow, fpos(inst->depot, j, inst), 1.0))
				print_error(" wrong CPXchgcoef [f2a]");
	}
	...
	// eq. (30)
	{
		...
		if (CPXnewrows(env, lp, 1, &rhs, &sense, NULL, cname))
			print_error(" wrong CPXnewrows [f1b]");
		for (int j = 0; j < inst->nnodes; j++)
			if (j != inst->depot && CPXchgcoef(env, lp, lastrow, fpos(j, inst->depot, inst), 1.0))
				print_error(" wrong CPXchgcoef [f1b]");
		if (inst->nveh <= 0 && CPXchgcoef(env, lp, lastrow, nvehpos, -inst->capacity))
			print_error(" wrong CPXchgcoef [f1c]");
	}
	...
	// eq. (31)
	{
		...
		if (CPXnewrows(env, lp, 1, &rhs, &sense, NULL, cname))
			print_error(" wrong CPXnewrows [f2a]");
		for (int j = 0; j < inst->nnodes; j++)
			if (j != inst->depot && CPXchgcoef(env, lp, lastrow, fpos(numext - 1, j, inst), 1.0))
				print_error(" wrong CPXchgcoef [f2b]");
		if (inst->nveh <= 0 && CPXchgcoef(env, lp, lastrow, nvehpos, -inst->capacity))
			print_error(" wrong CPXchgcoef [f2c]");
	}
	...
	for (int i = 0; i < numext; i++)
	{
		for (int j = i + 1; j < numext; j++)
		{
			...
			if (CPXnewrows(env, lp, 1, &rhs, &sense, NULL, cname))
				print_error(" wrong CPXnewrows [l1]");
			if (CPXchgcoef(env, lp, lastrow, ypos(i, j, inst), -inst->capacity))
				print_error(" wrong CPXchgcoef [l2]");
			if (CPXchgcoef(env, lp, lastrow, fpos(i, j, inst), 1.0))
				print_error(" wrong CPXchgcoef [l3]");
			if (CPXchgcoef(env, lp, lastrow, fpos(j, i, inst), 1.0))
				print_error(" wrong CPXchgcoef [l4]");
		}
	}
	...
	for (int i = 0; i < inst->nnodes; i++) // degree
	{
		...
		if (CPXnewrows(env, lp, 1, &rhs, &sense, NULL, cname))
			print_error(" wrong CPXnewrows [d1]");
		for (int j = 0; j < numext; j++)
			if (i != j && CPXchgcoef(env, lp, lastrow, ypos(i, j, inst), 1.0))
				print_error(" wrong CPXchgcoef [d2]");
	}
	...
	if (inst->nveh <= 0)
	{
		...
		if (CPXnewrows(env, lp, 1, &rhs, &sense, NULL, cname))
			print_error(" wrong CPXnewrows [v1]");
		if (CPXchgcoef(env, lp, lastrow, nvehpos, -1.0))
			print_error(" wrong CPXchgcoef [v2]");
		for (int j = 0; j < inst->nnodes; j++)
			if (j != inst->depot && CPXchgcoef(env, lp, lastrow, ypos(inst->depot, j, inst), 1.0))
				print_error(" wrong CPXchgcoef [v3]");
	}
...
/**************************************************************************************************************************/
{
	...
	for (int i = 0; i < inst->nnodes; i++)
	{
		...
		for (int j = i + 1; j < inst->nnodes; j++)
		{
			...
			if (CPXnewrows(env, lp, 1, &rhs, &sense, NULL, cname))
				print_error(" wrong CPXnewrows [e1]");
			if (CPXchgcoef(env, lp, lastrow, ypos(i, j, inst), -1.0))
				print_error(" wrong CPXchgcoef [e2]");
			if (CPXchgcoef(env, lp, lastrow, xpos(i, j, inst), 1.0))
				print_error(" wrong CPXchgcoef [e3]");
			if (CPXchgcoef(env, lp, lastrow, xpos(j, i, inst), 1.0))
				print_error(" wrong CPXchgcoef [e4]");
		}
	}
...
/**************************************************************************************************************************/
{
	...
	if (inst->nveh <= 0)
		print_error(" model 4 cannot be applied when n. vehicles is unspecified");
	...
	for (int i = 0; i < inst->nnodes; i++)
	{
		for (int j = i + 1; j < inst->nnodes; j++)
		{
			...
			double ub = 1.0;
			if (CPXnewcols(env, lp, 1, &obj, &zero, &ub, &binary, cname))
				print_error(" wrong CPXnewcols on z var.s");
			if (CPXgetnumcols(env, lp) - 1 != zpos(i, j, inst))
				print_error(" wrong position for z var.s");
		}
	}
	...
	for (int i = 0; i < inst->nnodes; i++) // degree
	{
		...
		sprintf(cname[0], "degree(%d)", i + 1);
		if (CPXnewrows(env, lp, 1, &rhs, &sense, NULL, cname))
			print_error(" wrong CPXnewrows [z1]");
		for (int j = 0; j < inst->nnodes; j++)
			if (i != j && CPXchgcoef(env, lp, lastrow, zpos(i, j, inst), 1.0))
...
void q_heur(instance *inst)
{
	...
	if (inst->model_type >= 2)
		print_error("q_heur() requires bigq var.s");
	...
	if (VERBOSE >= -100)
		for (int j = 0; j < inst->nnodes; j++)
			printf(" new ... qstar[%3d] = %10.2lf (%10.2lf -- %10.2lf)\n", j + 1, bigqstar[j], inst->load_min[j], inst->load_max[j]);
	free(bigqstar);
...
/***************************************************************************************************************************/
{
	...
	if (CPXgetx(env, lp, xstar, 0, ncols - 1))
		print_error("add_benders_cuts: no solution available?");

	// define the connected components (without the depot)
	int *comp = (int *)malloc(inst->nnodes * sizeof(int));
	for (int i = 0; i < inst->nnodes; i++)
		comp[i] = i; // all singletons
	for (int i = 0; i < inst->nnodes; i++)
	{
		if (i == inst->depot)
			continue;
		for (int j = i + 1; j < inst->nnodes; j++)
		{
			if (j == inst->depot)
				continue;
			if (xstar[zpos(i, j, inst)] < 0.5)
				continue; // xstar is 0-1
			if (comp[i] == comp[j])
				continue;
			int comp_i = comp[i], comp_j = comp[j];
			for (int h = 0; h < inst->nnodes; h++)
				if (comp[h] == comp_j)
					comp[h] = comp_i; // slow O(n^2) set union
		}
	}
	...
	for (int i = 0; i < inst->nnodes; i++)
	{
		if (i == inst->depot)
			continue;
		if (xstar[zpos(inst->depot, i, inst)] > 0.5)
			++nedges[comp[i]];
	...
	{
		int mycomp = comp[h];
		if (sum_demand[mycomp] < -1.0)
			continue; // means that the component was already considered
		int min_n_veh = (sum_demand[mycomp] / inst->capacity + 0.999);
		sum_demand[mycomp] = -2.0;
		if (nedges[mycomp] >= 2 * min_n_veh)
			continue;
		if (VERBOSE >= 100)
			printf("        ... violated SEC with n. edges %3d < %3d\n", nedges[mycomp], 2 * min_n_veh);
...
/**************************************************************************************************************************/
{
	...
	int heu = 1;
	CPXsetdblparam(env, CPX_PARAM_EPGAP, 1e-3); // CPXsetintparam(env, CPX_PARAM_INTSOLLIM, 1); //CPXsetintparam(env, CPX_PARAM_NODELIM, 10);
	...
	while (newcuts)
	{
		...
		} // CPXsetintparam(env, CPX_PARAM_NODELIM, 999999999); CPXsetintparam(env, CPX_PARAM_INTSOLLIM, 99999999); }

		if (VERBOSE >= 20)
			printf(" ... old Benders: iter. %3d lower bound %15.5lf in %10.3lf sec.s (tot. %10.3lf sec.s) new cuts %4d [nodes %4d heu %d]\n", ++iter, lb, second() - t1, second() - inst->tstart, newcuts, nn, heu);
	}
	if (VERBOSE >= 10)
		printf("\n ... old Benders: done after %4d iter.s (%5d cuts) in %10.3lf sec.s\n\n ################# OLD BENDERS DONE #########################\n\n", iter, ntot_cuts, second() - inst->tstart);
}

extern inline int imax(int i1, int i2);
extern inline double dmin(double d1, double d2);
extern inline double dmax(double d1, double d2);
